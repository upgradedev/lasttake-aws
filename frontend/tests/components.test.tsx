...
describe('scene workspace',()=>{
  ...
  it('filters script, opens evidence, inspects conflicting camera metadata',async()=>{const user=userEvent.setup();render(<SceneView scene={scene} state={state} selected={null}/>);expect(screen.getByRole('link',{name:/coverage · Shot plan advisory/})).toBeVisible();await user.click(screen.getByText('Slate 42A/1'));expect(screen.getByRole('table')).toContainHTML('OTHER');await user.selectOptions(screen.getByLabelText('Show'),'exceptions');expect(screen.queryByText('Delphine reacts')).not.toBeInTheDocument();await user.type(screen.getByRole('searchbox'),'missing');expect(screen.getByText(/No beats match/)).toBeVisible();});
...
describe('human authority',()=>{
  ...
  it('withholds pickup controls from other roles and rights acceptance from everyone',async()=>{render(<Actions scene={scene} state={state} role="production_coordinator" busy={false} act={act()}/>);expect(screen.queryByRole('button',{name:'Approve pickup'})).not.toBeInTheDocument();expect(screen.queryByRole('option',{name:'Accept the exception'})).not.toBeInTheDocument();expect(screen.getByText(/cannot accept away/)).toBeVisible();});

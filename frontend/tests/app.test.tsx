...
it('starts with useful empty state, creates a saved run and checks evidence',async()=>{server({empty:true,unchecked:true});const user=userEvent.setup();render(<App/>);expect(await screen.findByText('Know what still blocks wrap.')).toBeVisible();await user.click(screen.getByRole('button',{name:'Open guided demo'}));const guide=screen.getByText(/Walk through a fictional/).closest('section')!;expect(guide).toHaveTextContent('one Strands Agent');expect(guide).toHaveTextContent('eight @tool functions');expect(guide).toHaveTextContent('four bounded checks (tools, not agents)');expect(guide).toHaveTextContent('two ToolContext.interrupt approval transitions');expect(guide).toHaveTextContent('S3SessionManager');expect(guide).toHaveTextContent('only Bedrock mode adds two scoped interpreter Agent instances');expect(guide).toHaveTextContent('No mutation is queued or replayed');expect(guide).toHaveTextContent('receipt proves that subscriber ran, not that editorial acted');expect(guide).toHaveTextContent('Demo roles are unauthenticated');expect(guide).not.toHaveTextContent('no EventBridge rule or subscriber');await user.click(screen.getByRole('button',{name:'Start this fictional shoot day'}));await user.click(await screen.findByRole('button',{name:'Run wrap checkpoint'}));await screen.findByText('Checkpoint saved.');await user.click(screen.getByRole('button',{name:'Add take or release'}));expect(screen.getByText('Add evidence to this shoot day')).toBeVisible();await user.click(screen.getByRole('button',{name:'Close form'}));await user.click(screen.getByRole('button',{name:'Close guided demo'}));await user.click(screen.getByRole('link',{name:'Wrap status'}));expect(await screen.findByText('Current saved run')).toBeVisible();expect(localStorage.getItem('lasttake.session')).toBe(session.session_id);expect(localStorage.getItem('lasttake.run')).toBeNull();});

...
describe('architecture',()=>{
  it('names only the deployed tiers and their limits, and embeds the repository diagram',()=>{
    ...
    const diagram=screen.getByRole('img',{name:/browser keeps only a cached shell/});
    expect(diagram).toHaveAttribute('src','/architecture.svg');
    expect(diagram).toHaveAttribute('width','1200');
    expect(diagram).toHaveAttribute('height','920');
    for(const tier of ['Amazon S3 + Amazon CloudFront','Amazon API Gateway (HTTP API)','AWS Lambda (arm64) running the Strands Agents SDK','Amazon Aurora DSQL','Amazon EventBridge + terminal AWS Lambda'])expect(screen.getByText(tier)).toBeVisible();
    const text=document.body.textContent ?? '';
    expect(text).toContain('One Strands Agent');
    expect(text).toContain('eight @tool functions');
    expect(text).toContain('four tools, not four agents');
    expect(text).toContain('two ToolContext.interrupt transitions');
    expect(text).toContain('S3SessionManager');
    expect(text).toContain('two additional scoped interpreter Agents');
    expect(text).toContain('offline lexical reader');
    expect(text).toContain('not active in this browser');
    expect(text).toContain('No mutation is queued or replayed');
    expect(text).toContain('requires explicit review');
    expect(text).toContain('No staff authentication');
    expect(text).toContain('delivery-recorder');
    expect(text).toContain('receipt proves subscriber execution, not editorial completion');
    expect(text).toContain('never starts or resumes Strands');
    expect(text).not.toContain('no rule or subscriber is declared');

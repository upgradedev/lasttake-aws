...
export function App() {
  ...
  const [role, setRole] = useState<Role>(() => {
    const saved = readPreference('lasttake.role');
    return saved && Object.hasOwn(roles, saved) ? (saved as Role) : 'script_supervisor';
  });
  ...
  const navigateTo = useCallback(
    (page: Page) => {
      ...
      if (page !== 'overview') query.set('page', page);
      const activeRun = state?.run_id ?? route.run;
      if (activeRun) query.set('run', activeRun);
    ...
    <div className="app-shell">
      ...
      <aside className="navigation">
        <a className="brand" href={window.location.pathname} onClick={e => { e.preventDefault(); navigateTo('overview'); }}>
          ...
          <span>
            LASTTAKE<small>BEFORE THE SET COMES DOWN</small>
          </span>
        </a>
        ...
        <nav aria-label="Main navigation">
          {(['overview', 'scene', 'records', 'history'] as Page[]).map((key, index) => (
            <a
              key={key}
              href={key === 'overview' ? window.location.pathname : `?page=${key}`}
              onClick={e => {
                e.preventDefault();
                navigateTo(key);
              }}
          ...
          <a
            ...
            aria-current={route.page === 'journeys' ? 'page' : undefined}
          >
          ...
          <a
            ...
            aria-current={route.page === 'architecture' ? 'page' : undefined}
          >
          ...
          <a
            ...
            aria-current={route.page === 'roi' ? 'page' : undefined}
          >
            <span aria-hidden="true">◈</span>Production ROI
          </a>
        </nav>
        <div className="nav-bottom">
          <p className="synthetic">Synthetic demo</p>
          <p>
            One fictional production.<br />
            Evidence before decisions.
          </p>
          <button className="guide-toggle" aria-pressed={guided} onClick={() => setGuided(!guided)}>
            {guided ? 'Close guided demo' : 'Open guided demo'}
          </button>
        </div>
      </aside>
      ...
      <div className="workspace">
        <header className="topbar">
          <div className="topbar-title">
            <span className="production-dot" aria-hidden="true" />
            The Last Ferry <span className="muted">/ {scene?.scene_id ?? 'Fictional scene'}{scene?.revision ? ` · ${scene.revision}` : ''}</span>
          </div>
          <div className="execution-mode" data-testid="execution-mode">
            Synthetic demo · No footage/audio analysis or legal clearance. Demo roles are not staff authentication.{' '}
            <details>
              <summary>How this demo checks evidence</summary>
              Scripted planner · {state?.interpreter ?? 'Offline lexical interpreter'} · Real Strands approvals. The workflow reconciles supplied records and keeps material decisions with people.
            </details>
          </div>
            ...
            <label>
              ...
              <select
                ...
                onChange={e => {
                  ...
                  writePreference('lasttake.role', e.target.value);
                }}
              >
            ...
            <>
              <div className="page-heading">
                <div>
                  ...
                  <h1 id="page-title" tabIndex={-1}>
                    {!state && !route.run ? 'Know what still blocks wrap.' : pages[route.page]}
                  </h1>
                  <p>
                    {!state && !route.run
                      ? 'For the script supervisor and 1st AD: reconcile the shoot-day evidence, record the human wrap decision and give editorial a traceable handoff.'
                      : route.page === 'scene'
                      ? 'Reconcile the scene. Review the sources. Record the human decision.'
                      : route.page === 'records'
                      ? 'Inspect the supplied records and trace them back to the scene.'
                      : route.page === 'actions'
                      ? 'A decision belongs to a person and the exact evidence they reviewed.'
                      : route.page === 'history'
                      ? 'Saved runs, versioned handoffs and the event record.'
                      : 'Current scene, current run. Evidence before wrap.'}
                  </p>
                </div>
              ...
              {guided && (
                <section className="guide panel">
                  ...
                  <ol>
                    <li>Start the shoot day and run the wrap checkpoint.</li>
                  ...
                  <p>
                    This demo uses a scripted planner and an offline lexical interpreter with real Strands interrupts. No Bedrock inference, footage/audio analysis, email or payment occurs in these demo flows. AWS event-bus requests are real; acceptance does not establish downstream completion.
                  </p>
                </section>
              )}
              ...
              {w.busy && (
                <div className="loading" role="status">
                  ...
                  {w.progress || 'Reading the saved shoot day…'}
                </div>
              )}
              {w.error && (
                <div className="error" role="alert">
                  ...
                  <p>
                    {w.requiresRefresh
                      ? 'Refresh saved state before retrying a write. Your form entries are kept. Displayed evidence may be out of date.'
                      : 'The server refused the invalid request. Correct the supplied fields and submit again; your entries are kept.'}
                  </p>
                  <div className="toolbar">
                    <button disabled={w.busy} onClick={() => void w.refresh()}>Retry loading saved state</button>
                    {!state && <button disabled={w.busy} onClick={() => void w.recover()}>Start a separate session</button>}
                  </div>
                </div>
              )}
              {state?.needs_checkpoint && (
                <section className="warning" role="status">
                  <h2>Fresh checkpoint required</h2>
                  <p>{state.recovery_reason}</p>
                  {state.pending_approval ? (
                    <p>Open the pending approval in Workspace and decline it before checking again.</p>
                  ) : (
                    <button disabled={writesBlocked} onClick={() => void w.act('checkpoint')}>Run fresh checkpoint</button>
                  )}
                </section>
              )}
                ...
                <>
                  ...
                  {route.page === 'overview' && (
                    <Dashboard
                      ...
                      events={w.events}
                      busy={writesBlocked}
                      checkpoint={() => void w.act('checkpoint')}
                    />
                  )}
                  {(route.page === 'scene' || route.page === 'records') && (
                    <>
                      <div className="toolbar scene-actions">
                        ...
                        {!state.counts && (
                          <button className="primary" disabled={writesBlocked || !!state.pending_approval} onClick={() => void w.act('checkpoint')}>
                            Run wrap checkpoint
                          </button>
                        )}
                        <button disabled={w.busy} onClick={() => setIntake(!intake)} aria-expanded={intake}>
                          Add take or release
                        </button>
                      </div>
                      {intake && (
                        <Intake
                          ...
                          scene={scene}
                          busy={w.busy}
                          writeBlocked={w.requiresRefresh}
                          guided={guided}
                          onClose={() => setIntake(false)}
                          onSubmit={(kind, document) => w.act('ingest', { kind, document })}
                        />
                      )}
                    </>
                  )}
                  {route.page === 'scene' && (
                    <SceneView key={state.run_id} scene={scene} state={state} selected={route.beat} selection={selection} role={role} busy={writesBlocked} act={w.act} />
                  )}
                  {route.page === 'records' && (
                    <Records key={state.run_id} scene={scene} state={state} selection={selection} />
                  )}
                  {route.page === 'actions' && (
                    <>
                      <p className="scope-note">
                        This saved actions link remains available. <a href={link('scene', state.run_id, undefined, selection)}>Open the integrated workspace</a>
                      </p>
                      <Actions scene={scene} state={state} role={role} busy={writesBlocked} act={w.act} />
                    </>
                  )}
                  {route.page === 'history' && w.session && (
                    <History
                      key={state.run_id}
                      state={state}
                      session={w.session}
                      ...
                      act={w.act}
                      handle={w.handle}
                      historyLoading={w.historyLoading}
                      historyError={w.historyError}
                      olderRuns={w.olderRuns}
                      newestRuns={w.newestRuns}
                    />
                  )}
                  <footer>
                    <details>
                      <summary>Run details & execution labels</summary>
                      <dl className="metadata">
                        <dt>Saved run</dt><dd>{state.run_id}</dd>
                        <dt>Production / scene</dt><dd>{state.production_id} / {state.scene_id}</dd>
                        <dt>Framework</dt><dd>Strands Agents SDK</dd>
                        <dt>Planner</dt><dd>offline-scripted/1.0.0</dd>
                        <dt>Interpreter reported by API</dt><dd>{state.interpreter}</dd>
                        <dt>State store reported by API</dt><dd>{state.run_state_store}</dd>
                        <dt>Policy</dt><dd>{scene.policy_version}</dd>
                      </dl>
                      <p>Demo role selection is not staff authentication. The browser holds a session handle and preferences; records and decisions are stored by the backend.</p>
                    </details>
                    ...
                    <p><a href="/acceptance.html">Current automated acceptance</a> · <a href="/UAT.testbook.html">UAT testbook</a></p>
                  </footer>
                </>
              )}
            </>

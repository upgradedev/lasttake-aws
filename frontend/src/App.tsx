...
//
// One demo-mode statement, in the top bar, with the details behind a disclosure.
// It used to appear four times on the cold screen, and a warning repeated four
// times reads as boilerplate rather than as a limit worth knowing.
//
// Navigation named after the task: Wrap status, Scene review, Records, Handoff.
// The hash routes underneath are unchanged, so every saved link still works.

const NAV:Page[]=['overview','scene','records','history','journeys','architecture','roi'];
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      ...
      <main id="main" tabIndex={-1} aria-busy={w.busy}>
        ...
        {showLanding && <Landing session={w.session} busy={w.busy} page={route.page} start={()=>void w.create()}/>}

        {inRun && state && scene && !isDocPage && <>

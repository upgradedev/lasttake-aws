  ...
  return <div className="app-shell"><a href="#main" className="skip-link" onClick={e=>{e.preventDefault();document.getElementById('main')?.focus();}}>Skip to main content</a>
    <aside className="navigation"><a className="brand" href={link('overview',state?.run_id)}><span className="brand-mark" aria-hidden="true">L<span>◢</span></span><span>LASTTAKE<small>BEFORE THE SET COMES DOWN</small></span></a>
      ...
      <main id="main" tabIndex={-1} aria-busy={w.busy}><div className="page-heading"><div><p className="eyebrow">Shoot day · {scene?.revision ?? 'Fictional production'}</p><h1 id="page-title" tabIndex={-1}>{!state && !route.run?'Know what still blocks wrap.':pages[route.page]}</h1><p>{!state && !route.run?'For the script supervisor and 1st AD: reconcile the shoot-day evidence, record the human wrap decision and give editorial a traceable handoff.':route.page==='scene'?'Reconcile the scene. Review the sources. Record the human decision.':route.page==='records'?'Inspect the supplied records and trace them back to the scene.':route.page==='actions'?'A decision belongs to a person and the exact evidence they reviewed.':route.page==='history'?'Saved runs, versioned handoffs and the event record.':'Current scene, current run. Evidence before wrap.'}</p></div><div className="toolbar">{state && <button disabled={w.busy} onClick={()=>void w.refresh()}>Refresh saved state</button>}{showWelcome?<button className="primary" onClick={()=>void w.create()}>Start this fictional shoot day</button>:<button disabled={w.busy} onClick={()=>{setIntake(false);void w.create();}}>New shoot-day run</button>}</div></div>
        ...
        {showWelcome && <section className="welcome panel" aria-label="Start a shoot-day review"><p className="eyebrow">Find the gap while a pickup is still possible</p><h2>Bring the shoot day into focus.</h2><p>Start with The Last Ferry's script, take reports and releases. Follow each blocking reason to its evidence, resolve it with the responsible role, then request a separate wrap decision.</p><p><strong>Your result:</strong> a saved editorial turnover with the take map, human decisions and any accepted exceptions still visible.</p><p className="fine">Synthetic records are already supplied. No upload or account is required.</p><button onClick={()=>void w.create()}>New shoot-day run</button></section>}

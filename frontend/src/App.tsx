  ...
  return <div className="app-shell"><a href="#main" className="skip-link" onClick={e=>{e.preventDefault();document.getElementById('main')?.focus();}}>Skip to main content</a>
    <aside className="navigation"><a className="brand" href={link('overview',state?.run_id)}><span className="brand-mark" aria-hidden="true">L<span>◢</span></span><span>LASTTAKE<small>BEFORE THE SET COMES DOWN</small></span></a>
      ...
      <main id="main" tabIndex={-1} aria-busy={w.busy}><div className="page-heading"><div><p className="eyebrow">Shoot day · {scene?.revision ?? 'Fictional production'}</p><h1 id="page-title" tabIndex={-1}>{pages[route.page]}</h1><p>{route.page==='scene'?'Reconcile the scene. Review the sources. Record the human decision.':route.page==='records'?'Inspect the supplied records and trace them back to the scene.':route.page==='actions'?'A decision belongs to a person and the exact evidence they reviewed.':route.page==='history'?'Saved runs, versioned handoffs and the event record.':'Current scene, current run. Evidence before wrap.'}</p></div><div className="toolbar">{state && <button disabled={w.busy} onClick={()=>void w.refresh()}>Refresh saved state</button>}<button className="primary" disabled={w.busy} onClick={()=>{setIntake(false);void w.create();}}>New shoot-day run</button></div></div>
        ...
        <p className="execution-mode" data-testid="execution-mode">Synthetic demo · Scripted planner · {state?.interpreter ?? 'Offline lexical interpreter'} · Real Strands approvals. No footage/audio analysis or legal clearance. Demo roles are not staff authentication.</p>

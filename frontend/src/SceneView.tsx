...
import {beatMatches,hasApprovedPickup,reviewLabel,selectEvidence,uniqueBy} from './projection';
...
export function SceneView({scene,state,selected,selection={},role='script_supervisor',busy=false,act}:{scene:Scene;state:RunState;selected:string|null;selection?:Selection;role?:Role;busy?:boolean;act?:(path:string,extra?:Document)=>Promise<boolean>}) {
  ...
  return <>
    <div className="workspace-jumps" aria-label="Workspace panels">{[['script-pane','Script & takes'],['evidence-pane','Evidence'],['decision-pane','Decision']].filter(([id])=>act || id!=='decision-pane').map(([id,label])=><a key={id} href={'#'+id} onClick={e=>{e.preventDefault();const pane=document.getElementById(id);pane?.focus({preventScroll:true});pane?.scrollIntoView({block:'start'});}}>{label}</a>)}</div>
    ...
    <div className="scene-layout" data-testid="production-cockpit">
      <section id="script-pane" tabIndex={-1} className="script cockpit-pane" aria-label="Lined script"><div className="pane-heading"><h2>Scene & script beats</h2></div><div className="script-title"><div className="slate-index">{scene.scene_id}</div><div><h3>{scene.scene_heading}</h3><p>{revisionLabel(scene.revision)} · {scene.take_count} supplied takes</p></div></div>
        <div className="pane-controls"><label>Find a beat<input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Beat, text or page"/></label><label>Show<select value={['covered','exceptions','missing-releases'].includes(filter)?filter:'all'} onChange={e=>updateFilter(e.target.value)}><option value="all">All script beats</option><option value="exceptions">Beats with exceptions</option><option value="covered">Covered required beats</option><option value="missing-releases">Beats missing release records</option></select></label></div>
      ...
      {act && <section id="decision-pane" tabIndex={-1} className="cockpit-pane decision-console" aria-label="Role and decision console"><div className="pane-heading"><h2>Human decision</h2></div><div className="wrap-status" data-testid="wrap-status"><span>Eligibility <strong className={state.counts && state.eligible?'state-good':'state-warn'}>{!state.counts?'Not assessed':state.eligible?'Eligible':'Blocked'}</strong></span><span>Wrap <strong className={state.wrap_approved?'state-good':'state-warn'}>{state.wrap_approved?'Approved':'Not approved'}</strong></span>{state.pending_approval && <a href={link('scene',state.run_id,undefined,{filter:'approval'})}>Review pending {state.pending_approval.reason.kind} · {roles[state.pending_approval.reason.required_role]}{state.pending_approval.evidence_changed?' · evidence changed':''}</a>}</div><div className="pane-scroll" key={filter==='approval'?'approval':'finding'} tabIndex={0} aria-label="Decision controls"><p className="console-role">Reviewing as {roles[role]}</p>
        {/* Once wrap is approved, the wrap result leads; an earlier finding card follows it. */}
        {state.wrap_approved && <ApprovalConsole state={state} role={role} busy={busy} act={act}/>}
        {filter!=='approval' && (finding && !invalid?<DecisionForm key={finding.finding_id+':'+finding.record_sha256+':'+role} compact finding={finding} scene={scene} state={state} role={role} busy={busy} act={act}/>:<p className="empty">{!state.counts?'Run the wrap checkpoint first. Findings to review will appear here.':state.wrap_approved?'Select a current finding to review its decision.':'Select a current finding to review its decision. Approval controls remain separate below.'}</p>)}
        {!state.wrap_approved && <ApprovalConsole state={state} role={role} busy={busy} act={act}/>}

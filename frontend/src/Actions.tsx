...
import {decisionFor,link,roles,words} from './model';
...
export function DecisionForm({finding,scene,state,role,busy,act,index=0,compact=false}:{finding:Finding;scene:Scene;state:RunState;role:Role;busy:boolean;act:Act;index?:number;compact?:boolean}) {
  ...
  return <article className="panel decision-card" data-check={finding.check_type} data-review={current.decision && !current.stale?'recorded':'open'} style={{animationDelay:`${Math.min(index,4)*60}ms`}} aria-label={`${words(finding.check_type)} ${finding.requirement_id ?? 'advisory'}`}>
    {compact?<><p className="eyebrow">{words(finding.check_type)} decision</p><h3>{finding.requirement_id ?? 'Shot plan advisory'}</h3><details><summary>Record id</summary><p className="fine">{finding.finding_id}</p></details></>:<Evidence finding={finding} scene={scene} decisions={state.decisions}/>}
...
export function ApprovalConsole({state,role,busy,act}:{state:RunState;role:Role;busy:boolean;act:Act}) {
  ...
  // After the 1st AD approves, the next step is the handoff, not another
  // readiness review. The handoff link needs what publishing needs
  // (Turnover.tsx) and what WorkflowNext needs before it names the handoff: a
  // current assessment that is eligible. A renewed request keeps the review
  // controls, and so does an approval with no current eligible assessment.
  const approved=state.wrap_approved && !pending && Boolean(state.counts) && state.eligible;
  return <><section className="panel approvals" aria-labelledby="approval-title" data-state={state.wrap_approved?'recorded':pending?'waiting':'review'}><div className="approval-heading"><div><p className="eyebrow">Human authority</p><h2 id="approval-title">Pickup & wrap approvals</h2></div><span className="authority-label">1st AD · final decision</span></div>
    <ul className="approval-stages" aria-label="Wrap decision stages"><li data-complete={Boolean(state.counts)}>Evidence {state.counts?'assessed':'not assessed'}</li><li data-complete={Boolean(state.counts)&&state.eligible}>Gate {!state.counts?'not assessed':state.eligible?'eligible':'not eligible'}</li><li data-complete={state.wrap_approved}>Wrap {state.wrap_approved?'approved':'not approved'}</li></ul>
    ...
    <div className="toolbar">{approved?<a className="button primary" href={link('history',state.run_id)}>{state.turnover?'Read the saved turnover':'Publish the approved turnover next'}</a>:<><button disabled={busy || !state.counts} onClick={()=>void act('evaluate')}>Review wrap readiness</button>{state.counts && state.eligible && !pending && !state.wrap_approved && role==='first_ad' && <button className="primary" disabled={busy} onClick={()=>void act('wrap',{role})}>Request wrap approval</button>}</>}</div>
    {!state.counts && <p className="fine">Run a checkpoint first to assess readiness.</p>}
    {state.counts && !approved && <p className={state.eligible?'verified-text':'warning'}>{state.eligible?'The evidence gate reports eligible. Only the 1st AD can approve wrap.':`${state.causes.length} open cause(s) prevent wrap eligibility.`}</p>}

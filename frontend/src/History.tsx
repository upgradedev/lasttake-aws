...
export function History({state,session,events,role,busy,act,handle}:{state:RunState;session:Session;events:EventRow[];role:Role;busy:boolean;act:(path:string,extra?:Document)=>Promise<boolean>;handle:(work:()=>Promise<void>)=>Promise<boolean>}) {
  ...
  </section><section className="panel"><p className="eyebrow">Portable decision record</p><h2>Receipt for {roles[role]}</h2><p>A receipt carries the current run, policy, evidence digests and every open item. Accepted exceptions remain open on the receipt.</p>
    <div className="toolbar"><label>Receipt purpose<select value={kind} onChange={e=>{setKind(e.target.value);setReceipt(null);}}><option value="pickup">Pickup review</option><option value="wrap">Wrap review</option></select></label><button disabled={busy || !state.counts} onClick={()=>void handle(async()=>{const data=await request<{receipt:Receipt}>('receipt',{run_id:state.run_id,session_id:session.session_id,kind});setReceipt(data.receipt);})}>Prepare receipt</button></div>

...
- The ci.yml job "interrupt survives process death" runs `lasttake checkpoint` and `lasttake approve --yes --hours 7.5` as two separate `run:` steps, which are two operating system processes. Between them, a negative control moves `.lasttake/sessions` aside and answers the pending interrupt by its id. It passes only when Strands refuses with `not in interrupt state`, and only when every file hashes the same after the session is put back, so process 2 resumes from exactly what process 1 left. No step compares the two process ids or checks the approval message. [Interrupt and resume across process death](docs/strands-interrupt-resume.md#in-ci-two-processes-and-a-negative-control) lists each step and what the job does not show.
...
**A rewritten case.** At this commit `tools/measure.py` reports all 8 cases matching and exits 0; no workflow runs it. Commit 911fa59 changed `WrapRun.publish` so that a publish that raises is recorded as outcome unknown and not resent. The retry-and-recovery case still expected that resend, so the tool reported 7 of 8 cases matching until the case was rewritten to retry after a definite rejection. The retired expectation and that 7 of 8 run are kept in `REWRITES` in the tool and under `history` in `docs/measurement.json`. From any directory, a run rewrites the repository's `docs/measurement.json` and keeps its `_current_scope` and `history` blocks. Details are in [Evaluation harness](docs/evaluation-harness.md#a-baseline-written-before-the-run).
